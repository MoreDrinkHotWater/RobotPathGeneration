...
void CGroundExtraction::getCloudBound(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pmProcessUrban::Bounds &bound) {
...
void CGroundExtraction::CalculateGridAndMinZ(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
    ...
    for (int i = 0; i < num_voxel; i++) {
        ...
        if (ExpandGrid_TempList == 1 || ExpandGrid_TempRow == 1 || ExpandGrid_TempList == list
                || ExpandGrid_TempRow == row) {
    ...
    for (int i = 0; i < num_voxel; i++) {
        ...
        for (int j = -1; j < 2; j++) {
            for (int k = -1; k < 2; k++) {
                if (grid[i].NeighborMin_z > temp_grid[ExpandGrid_TempNum + j * ExpandGrid_List + k].min_z
                        && (j != 0 || k != 0))
    ...
    for (int i = 0; i < num_voxel; i++) {
        for (int j = 0; j < grid[i].point_id.size(); j++) {
            if (cloud->points[grid[i].point_id[j]].z - grid[i].min_z < max_height_difference_
                    && grid[i].min_z - grid[i].NeighborMin_z < 2 * grid_resolution_) {
...
void CGroundExtraction::FilterNotGroundPoint(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
    ...
    while (!Unsearch.empty()) {
        ...
        Kdtree_search_ground_cloud.radiusSearch(Searchpoint,
                                                radius,
                                                PointIdSearch_ground_cloud,
                                                PointDistanceSearch_ground_cloud);
        if (PointIdSearch_ground_cloud.size() < 5) {
            ...
            if (PointIdSearch_ground_cloud.size() > 10) {
                ...
                if (PointIdSearch_cloud.size() > 2 * PointIdSearch_ground_cloud.size()) {
                    for (int i = 0; i < PointIdSearch_ground_cloud.size(); i++) {
                        if (isearse[PointIdSearch_ground_cloud[i]] != -1) {
                            is_ground_points[PointIdSearch_ground_cloud[i]] =
                                    -1;//����ԭʼ�������������ĵ������ڵ�������������ĵ����Ķ��������޸�Ϊ�ǵ����;
                            Unsearch.erase(PointIdSearch_ground_cloud[i]);
                            isearse[PointIdSearch_ground_cloud[i]] = -1;
                        }
